"""SKIDS-011 — Makar fixture asset tests (proof fixture, not final art)."""
...
_BUNDLE = Path(__file__).resolve().parents[2] / "library" / "characters" / "makar"
_ART = _BUNDLE / "art" / "front.svg"
_SPEC_PATH = _BUNDLE / "character.yaml"
_RIG_PATH = Path(__file__).resolve().parents[2] / "library" / "rig_profiles" / "fox_cartoon.yaml"
...
class TSpec(unittest.TestCase):
    ...
    def test_02_id_makar(self):
        self.assertEqual(_spec()["id"], "makar")

    def test_03_species_fox(self):
        self.assertEqual(_spec()["species"], "fox")

    def test_04_rig_fox_cartoon(self):
        self.assertEqual(_spec()["rig_profile"], "fox_cartoon")
...
class TArt(unittest.TestCase):
    ...
    def test_06_root_rig_profile(self):
        self.assertEqual(ET.parse(_ART).getroot().get("data-rig-profile"), "fox_cartoon")
    ...
    def test_12_curious_expression(self):
        self.assertIn("curious", _svg_info()["expressions"])
...
class TRenderer(unittest.TestCase):
    ...
    def test_23_idle_render(self):
        ...
        inst = [e for e in root.iter()
                if e.get("data-character-instance") == "makar"]
    ...
    def test_26_point_state(self):
        segs = [
            {"start_ms": 0, "end_ms": 180, "pose": "idle", "viseme": "REST"},
            {"start_ms": 180, "end_ms": 1000, "pose": "point", "viseme": "REST"},
            {"start_ms": 1000, "end_ms": 1260, "pose": "idle", "viseme": "REST"},
        ]
        tl = _tl(dur=1260, segs=segs)
        point_r = _parse_frame(render_frame(_BUNDLE, 500, 700, 400, _chars(tl)))
        self._assert_layer(point_r, "data-expression", "curious")
        self._assert_layer(point_r, "data-gaze", "right")
        self._assert_variant(point_r, "pupil_left", "right")
        self._assert_variant(point_r, "pupil_right", "right")
        arm = [e for e in point_r.iter() if e.get("data-part") == "arm_right"]
        roots = [e for e in arm[0].iter() if e.get("data-motion-root") is not None]
        self.assertEqual(roots[0].get("transform"), "rotate(-45)")
